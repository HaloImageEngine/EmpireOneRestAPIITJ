/* SOURCE: jasmine.arvixe.com  (DB: LuxDiablo)
   OUTPUT: ONE big INSERT script for dbo.LotteryDrawResultsMega (preserves ResultID)
  ...
  @values =
    STRING_AGG(
      CAST(
        ...
        /* Num1..Num5 (tinyint) */           COALESCE(CAST([Num1] AS nvarchar(3)), N'NULL') + N',' +
        ...
        /* UNum1..UNum5 (tinyint) */         COALESCE(CAST([UNum1] AS nvarchar(3)), N'NULL') + N',' +
...
FROM dbo.LotteryDrawResultsMega;

-- Safety: empty table
IF @values IS NULL OR LEN(@values) = 0
BEGIN
  SELECT N'-- LotteryDrawResultsMega has no rows to export.' AS [-- Script];
  RETURN;
END
...
SET IDENTITY_INSERT dbo.LotteryDrawResultsMega ON;

INSERT INTO dbo.LotteryDrawResultsMega
([ResultID],[UserID],[UserAlias],[CardName],[GameName],[CardSlot],[WID],[DrawDate],
 [Num1],[Num2],[Num3],[Num4],[Num5],[PNum],
 [UNum1],[UNum2],[UNum3],[UNum4],[UNum5],[UPNum],
 [Winner],[RequestMatch],[ActualMatch],
 [CreatedAt],[EMailSent])
VALUES
' + @values + N';

SET IDENTITY_INSERT dbo.LotteryDrawResultsMega OFF;
