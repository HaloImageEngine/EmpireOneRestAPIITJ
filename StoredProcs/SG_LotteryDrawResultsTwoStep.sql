/* SOURCE: jasmine.arvixe.com  (DB: LuxDiablo)
   OUTPUT: ONE big INSERT script for dbo.LotteryDrawResultsTwoStep (preserves ResultID)
  ...
  @values =
    STRING_AGG(
      CAST(
        ...
        /* Num1..Num4 (tinyint) */           COALESCE(CAST([Num1] AS nvarchar(3)), N'NULL') + N',' +
        ...
        /* PNum (tinyint) */                 COALESCE(CAST([PNum] AS nvarchar(3)), N'NULL') + N',' +
        /* UNum1..UNum4 (tinyint) */         COALESCE(CAST([UNum1] AS nvarchar(3)), N'NULL') + N',' +
        ...
        /* UPNum (tinyint) */                COALESCE(CAST([UPNum] AS nvarchar(3)), N'NULL') + N',' +
...
FROM dbo.LotteryDrawResultsTwoStep;

-- Safety: empty table
IF @values IS NULL OR LEN(@values) = 0
BEGIN
  SELECT N'-- LotteryDrawResultsTwoStep has no rows to export.' AS [-- Script];
  RETURN;
END
...
SET IDENTITY_INSERT dbo.LotteryDrawResultsTwoStep ON;

INSERT INTO dbo.LotteryDrawResultsTwoStep
([ResultID],[UserID],[UserAlias],[CardName],[GameName],[CardSlot],[WID],[DrawDate],
 [Num1],[Num2],[Num3],[Num4],[PNum],
 [UNum1],[UNum2],[UNum3],[UNum4],[UPNum],
 [Winner],[RequestMatch],[ActualMatch],
 [CreatedAt],[EMailSent])
VALUES
' + @values + N';

SET IDENTITY_INSERT dbo.LotteryDrawResultsTwoStep OFF;
